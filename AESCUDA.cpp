...
int main(){
    ...
    printf("ENCRYPTION:\n");
     clock_t start, end; // Added clock_t variables
...
__global__ void AES_main(uint8_t *input, uint8_t *keySchedule, uint8_t *output) {
    int idx = blockIdx.x * blockDim.x + threadIdx.x;
    int stride = blockDim.x * gridDim.x;
    int encrypting = 1; // Set to 1 for encryption, 0 for decryption
    
    for (int i = idx; i < 4 * Nb; i += stride) {
        output[i] = input[i];
    }
    
    if (idx == 0) {
        Cipher(output, keySchedule, keySize, encrypting);
    }
}
...
void encrypt(char* plaintext, char* keyStr){
    ...
    uint8_t *keySchedule = NULL;
    cudaMallocManaged(&keySchedule, 4 * Nb * (Nr(keySize) + 1) * sizeof(uint8_t));
    ...
    uint8_t *output = NULL;
    cudaMallocManaged(&output, 4 * Nb * sizeof(uint8_t));
    
    /* Run cipher */
    dim3 blockDim(256, 1, 1);
    dim3 gridDim((4 * Nb + blockDim.x - 1) / blockDim.x, 1, 1);
    AES_main<<<gridDim, blockDim>>>(input, keySchedule, output);
...
uint8_t galoisMultiply(uint8_t a, uint8_t b){
    /*
        ...
        Implementation based on description from https://e...content-available-to-author-only...a.org/wiki/Finite_field_arithmetic#Rijndael's_(AES)_finite_field
    */
    uint8_t p = 0;
    int i;
    int carry;
...
uint8_t* Rcon(int a){
    /* Calculates the round constant and returns it in an array.
       ...
       https://g...content-available-to-author-only...b.com/secworks/aes/blob/6fb0aef25df082d68da9f75e2a682441b5f9ff8e/src/model/python/rcon.py#L180
    */
    uint8_t rcon = 0x8d;
    int i;
    for(i = 0; i < a; i++){
